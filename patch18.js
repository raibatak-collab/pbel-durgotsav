...
const target1 = `{/* Dynamic QR Scanner & 1-Tap Mobile Payment Widget */}`;
content = content.replace(target1, `{!isUatTest && (<div className="test-wrapper">${target1}`);

const target2 = `<label className="block text-xs font-semibold text-gray-700 uppercase mb-1">
                    UPI UTR / Transaction Ref No. (Optional from GPay/PhonePe)
                  </label>`;
content = content.replace(target2, `</div>)}
                  {!isUatTest && (<div>${target2}`);

const target3 = `onChange={(e) => setCustomFormData({ ...customFormData, upiRef: e.target.value })}
